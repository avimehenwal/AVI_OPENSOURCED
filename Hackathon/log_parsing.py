...
pattern = r'(\w+) "([/\w_\-\,;\%\?=\&\.\(\)\*\:]+)" for ([\d\.]+) ([\w\s\.\n:\+\S\W]+)'
patt = r'\w+ (\d+ \w+)'

for i in log:
    item = i.strip()
    print 'LOG-ITEM', item, type(item), len(item)
    if len(item) < 5:
        continue
    else:
        print '='*50
        ...
        if len(m.group(1)) > 0: REQUEST_TYPE += 1
        if len(m.group(2)) > 0: RELATIVE_URL += 1
        if len(m.group(3)) > 0: IP += 1
        print REQUEST_TYPE, RELATIVE_URL, IP

        part = m.group(4).split('\n')
        print '-'*50
        print len(part), part
        print part[-1]

        s = re.search(patt, part[-1])

        print s.group(1)
        if len(m.group(3)) > 0: RESPONSE_CODE += 1
